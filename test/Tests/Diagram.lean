...
def testGetEnv_rect : IO Unit := do
  ...
  assertApproxEq env[Vec2.east].diag 3.5 "rect env east"
  assertApproxEq env[Vec2.north].diag 2.5 "rect env north"

def testGetEnv_circle : IO Unit := do
  let d : Diagram SVG := Diagram.circle 5
  let env := d.getEnvelope
  assertApproxEq env[Vec2.east].diag 5.5 "circle env east" (tol := 0.01)
  assertApproxEq env[Vec2.north].diag 5.5 "circle env north" (tol := 0.01)

def testGetEnv_empty : IO Unit := do
  let d : Diagram SVG := .empty
  let env := d.getEnvelope
  assertApproxEq env[Vec2.east].diag 0 "empty env east"

def testGetEnv_transformed : IO Unit := do
  let d : Diagram SVG := .transform (Matrix.translate 10 0) (Diagram.rect 2 2)
  let env := d.getEnvelope
  assertApproxEq env[Vec2.east].diag 11.5 "translated rect env east"
  assertApproxEq env[Vec2.west].diag (-8.5) "translated rect env west"
...
def testGetEnv_compose : IO Unit := do
  ...
  let d := Diagram.compose a b
  let env := d.getEnvelope
  assertApproxEq env[Vec2.east].diag 2.5 "compose env east = max(2,1)"
  assertApproxEq env[Vec2.north].diag 3.5 "compose env north = max(1,3)"
...
def testHcomp_envelopeWidth : IO Unit := do
  ...
  assertApproxEq env[Vec2.east].diag 6 "hjoin east" (tol := 0.01)
  assertApproxEq env[Vec2.west].diag 6 "hjoin west" (tol := 0.01)
...
def testHcomp_height : IO Unit := do
  ...
  let d := Diagram.hjoin a b
  let env := d.getEnvelope
  assertApproxEq env[Vec2.north].diag 4.5 "hjoin north = max height" (tol := 0.01)
...
def testVcomp_envelopeHeight : IO Unit := do
  ...
  assertApproxEq env[Vec2.north].diag 6 "vjoin north" (tol := 0.01)
  assertApproxEq env[Vec2.south].diag 6 "vjoin south" (tol := 0.01)
...
def testBeside_withGap : IO Unit := do
  ...
  assertApproxEq env[Vec2.east].diag 7.5 "beside gap east" (tol := 0.01)

def testHcomp_empty : IO Unit := do
  let a : Diagram SVG := Diagram.rect 4 2
  let d := Diagram.hjoin a (Diagram.emptyDiagram : Diagram SVG)
  let env := d.getEnvelope
  assertApproxEq env[Vec2.east].diag 2.5 "hjoin with empty east" (tol := 0.01)
...
def testHcat_three : IO Unit := do
  ...
  assertApproxEq env[Vec2.east].diag 4.5 "hcat three east" (tol := 0.01)
...
def testVcat_two : IO Unit := do
  ...
  assertApproxEq env[Vec2.south].diag 5 "vcat two south" (tol := 0.01)

def testHcat_empty : IO Unit := do
  let d : Diagram SVG := Diagram.hcat []
  let env := d.getEnvelope
  assertApproxEq env[Vec2.east].diag 0 "hcat empty"
...
def testVcat_single : IO Unit := do
  ...
  assertApproxEq env[Vec2.south].diag 2.5 "vcat single south" (tol := 0.01)
...
def testHcat_singleBox : IO Unit := do
  ...
  assertApproxEq env[Vec2.east].diag 2.5 "hcat single east" (tol := 0.01)
...
def testGrid_2x2 : IO Unit := do
  ...
  assertApproxEq env[Vec2.east].diag 3 "grid 2x2 east" (tol := 0.1)
...
def testGrid_withNone : IO Unit := do
  ...
  assertTrue (env[Vec2.east].diag > 0) "grid with holes has positive extent"
...
def testGrid_singleCell : IO Unit := do
  ...
  assertApproxEq env[Vec2.east].diag 3.5 "grid single east" (tol := 0.1)
...
def testGrid_1x3 : IO Unit := do
  ...
  assertApproxEq env[Vec2.east].diag 4.5 "grid 1x3 east" (tol := 0.1)
...
def testAnchor_zeroEnvelope : IO Unit := do
  ...
  let env := d.getEnvelope
  assertApproxEq env[Vec2.east].diag 0 "anchor zero east"
  assertApproxEq env[Vec2.north].diag 0 "anchor zero north"
...
def testAnchor_composedEnvelope : IO Unit := do
  ...
  let env := d.getEnvelope
  assertApproxEq env[Vec2.east].diag 2.5 "anchor in compose doesn't change envelope"

def testNamed_preservesEnvelope : IO Unit := do
  let d : Diagram SVG := .named `box (Diagram.rect 6 4)
  let env := d.getEnvelope
  assertApproxEq env[Vec2.east].diag 3.5 "named preserves east"
  assertApproxEq env[Vec2.north].diag 2.5 "named preserves north"
...
def testFloating_zeroEnvelope : IO Unit := do
  ...
  let env := d.getEnvelope
  assertApproxEq env[Vec2.east].diag 0 "floating zero east"
  assertApproxEq env[Vec2.north].diag 0 "floating zero north"

def testStrut_envelope : IO Unit := do
  let d : Diagram SVG := Diagram.strut (Envelope.ofRect 5 3)
  let env := d.getEnvelope
  assertApproxEq env[Vec2.east].diag 5 "strut east"
  assertApproxEq env[Vec2.north].diag 3 "strut north"
...
def testWithEnvelope_override : IO Unit := do
  ...
  let env := d.getEnvelope
  assertApproxEq env[Vec2.east].diag 100 "withEnvelope override east"
...
def testFloating_inCompose : IO Unit := do
  ...
  assertApproxEq env[Vec2.east].diag 2.5 "floating doesn't extend hjoin" (tol := 0.01)
...
def testPad_uniform : IO Unit := do
  ...
  let env := d.getEnvelope
  assertApproxEq env[Vec2.east].diag 5.5 "pad east 2+3"
  assertApproxEq env[Vec2.north].diag 4.5 "pad north 1+3"

def testPadRight_only : IO Unit := do
  let d : Diagram SVG := Diagram.padRight 5 (Diagram.rect 4 2)
  let env := d.getEnvelope
  assertApproxEq env[Vec2.east].diag 7.5 "padRight east 2+5"
  assertApproxEq env[Vec2.west].diag 2.5 "padRight west unchanged"

def testPadLRTB : IO Unit := do
  let d : Diagram SVG := Diagram.padLRTB 1 2 3 4 (Diagram.rect 4 2)
  let env := d.getEnvelope
  assertApproxEq env[Vec2.west].diag 3.5 "padLRTB west 2+1"
  assertApproxEq env[Vec2.east].diag 4.5 "padLRTB east 2+2"
  assertApproxEq env[Vec2.north].diag 4.5 "padLRTB north 1+3"
  assertApproxEq env[Vec2.south].diag 5.5 "padLRTB south 1+4"

def testPadXY : IO Unit := do
  let d : Diagram SVG := Diagram.padXY 2 3 (Diagram.rect 4 2)
  let env := d.getEnvelope
  assertApproxEq env[Vec2.east].diag 4.5 "padXY east 2+2"
  assertApproxEq env[Vec2.north].diag 4.5 "padXY north 1+3"

def testPad_zero : IO Unit := do
  let d : Diagram SVG := Diagram.pad 0 (Diagram.rect 4 2)
  let env := d.getEnvelope
  assertApproxEq env[Vec2.east].diag 2.5 "pad zero east unchanged"
...
def testSetEnvelopeRight : IO Unit := do
  ...
  let env := d.getEnvelope
  assertApproxEq env[Vec2.east].diag 10 "setEnvelopeRight east"
  assertApproxEq env[Vec2.west].diag 2.5 "setEnvelopeRight west unchanged"

def testSetEnvelopeTop : IO Unit := do
  let d : Diagram SVG := Diagram.setEnvelopeTop 20 (Diagram.rect 4 2)
  let env := d.getEnvelope
  assertApproxEq env[Vec2.north].diag 20 "setEnvelopeTop north"

def testHGap : IO Unit := do
  let d : Diagram SVG := .hgap 8
  let env := d.getEnvelope
  assertApproxEq env[Vec2.east].diag 4 "hGap east = half width"
  assertApproxEq env[Vec2.north].diag 0 "hGap zero height"

def testVGap : IO Unit := do
  let d : Diagram SVG := .vgap 6
  let env := d.getEnvelope
  assertApproxEq env[Vec2.north].diag 3 "vGap north = half height"
  assertApproxEq env[Vec2.east].diag 0 "vGap zero width"
...
def testHGap_inCompose : IO Unit := do
  ...
  assertApproxEq env[Vec2.east].diag 8 "hGap in compose east" (tol := 0.01)
...
def testAlignFraction_bottom : IO Unit := do
  ...
  assertApproxEq env[Vec2.north].diag 6.5 "align bottom north" (tol := 0.01)
  assertApproxEq env[Vec2.south].diag 2.5 "align bottom south" (tol := 0.01)
...
def testAlignFraction_top : IO Unit := do
  ...
  assertApproxEq env[Vec2.north].diag 2.5 "align top north" (tol := 0.01)
  assertApproxEq env[Vec2.south].diag 6.5 "align top south" (tol := 0.01)
...
def testAlignFraction_center : IO Unit := do
  ...
  assertApproxEq env[Vec2.north].diag 4.5 "align center north" (tol := 0.01)
  assertApproxEq env[Vec2.south].diag 4.5 "align center south" (tol := 0.01)
...
def testAlignFraction_sameHeight : IO Unit := do
  ...
  assertApproxEq env[Vec2.north].diag 2.5 "align same height north" (tol := 0.01)
  assertApproxEq env[Vec2.south].diag 2.5 "align same height south" (tol := 0.01)
...
def testAlignAnchor_fallback : IO Unit := do
  ...
  let env := d.getEnvelope
  assertApproxEq env[Vec2.north].diag 4.5 "anchor fallback north" (tol := 0.01)
...
def testHcat_topAlign : IO Unit := do
  ...
  assertApproxEq env[Vec2.north].diag 4.5 "hcat top north" (tol := 0.01)
  assertApproxEq env[Vec2.south].diag 4.5 "hcat top south" (tol := 0.01)
...
def testHcat_bottomAlign : IO Unit := do
  ...
  assertApproxEq env[Vec2.north].diag 4.5 "hcat bottom north" (tol := 0.01)
  assertApproxEq env[Vec2.south].diag 4.5 "hcat bottom south" (tol := 0.01)
...
def testVcat_leftAlign : IO Unit := do
  ...
  assertApproxEq env[Vec2.east].diag 4.5 "vcat left east" (tol := 0.01)
  assertApproxEq env[Vec2.west].diag 4.5 "vcat left west" (tol := 0.01)
...
def testVcat_rightAlign : IO Unit := do
  ...
  assertApproxEq env[Vec2.east].diag 4.5 "vcat right east" (tol := 0.01)
  assertApproxEq env[Vec2.west].diag 4.5 "vcat right west" (tol := 0.01)
...
def testHsep_topAlign : IO Unit := do
  ...
  assertApproxEq env[Vec2.north].diag 4.5 "hsep top north" (tol := 0.01)
  assertApproxEq env[Vec2.south].diag 4.5 "hsep top south" (tol := 0.01)
...
def testVsep_leftAlign : IO Unit := do
  ...
  assertApproxEq env[Vec2.east].diag 4.5 "vsep left east" (tol := 0.01)
  assertApproxEq env[Vec2.west].diag 4.5 "vsep left west" (tol := 0.01)
