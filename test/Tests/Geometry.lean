...
def testEmpty_arbitrary : IO Unit := do
  assertApproxEq (Envelope.empty ⟨0.3, 0.95⟩) 0 "empty arbitrary"

def testEmpty_unionIdentity : IO Unit := do
  let env := Envelope.ofRect 3 2
  assertApproxEq (Envelope.union Envelope.empty env Vec2.east) 3 "empty is union identity (east)"
  assertApproxEq (Envelope.union Envelope.empty env Vec2.north) 2 "empty is union identity (north)"
...
def testCircle_arbitrary : IO Unit := do
  assertApproxEq (Envelope.ofCircle 3 ⟨0.6, 0.8⟩) 3 "circle arbitrary direction"
...
def testUnion_contained : IO Unit := do
  ...
  let u := Envelope.union small big
...
def testUnion_commutative : IO Unit := do
  ...
  assertApproxEq (Envelope.union a b Vec2.east) (Envelope.union b a Vec2.east) "union commutative east"
  assertApproxEq (Envelope.union a b Vec2.north) (Envelope.union b a Vec2.north) "union commutative north"
...
def testAnchors_symmetric : IO Unit := do
  let anchors := CardinalAnchors.fromEnvelope (Envelope.ofRect 3 2)
...
def testAnchors_translated : IO Unit := do
  let anchors := CardinalAnchors.fromEnvelope (Envelope.translateBy ⟨5, 3⟩ (Envelope.ofRect 1 1))
  assertVec2Eq anchors.center ⟨5, 3⟩ "translated center"

def testAnchors_circle : IO Unit := do
  let anchors := CardinalAnchors.fromEnvelope (Envelope.ofCircle 4)
...
def testAnchors_square : IO Unit := do
  let anchors := CardinalAnchors.fromEnvelope (Envelope.ofRect 5 5)
