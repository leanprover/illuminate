...
/-- Draws a normal (non-accepting) state at position `{lit}i` with the given label. -/
...
/-- Draws an accepting state (double circle) at position `{lit}i` with the given label. -/
...
/-- Draws a start arrow pointing into state `{lit}i`. -/
...
/-- Draws a straight labeled edge from state `{lit}i` to state `{lit}j`. -/
...
/-- Draws a curved labeled arc from state `{lit}i` to state `{lit}j`, bending up or down. -/
...
/-- Draws a self-loop on state `{lit}i` with the given label. -/
